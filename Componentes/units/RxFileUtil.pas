...
function FileLock(Handle: THandle; Offset, LockSize: LongInt): Integer;
{$IFDEF RX_D4}overload; {$ENDIF}
{$IFDEF RX_D4}
function FileLock(Handle: THandle; Offset, LockSize: Int64): Integer; overload;
{$ENDIF}
function FileUnlock(Handle: THandle; Offset, LockSize: LongInt): Integer;
{$IFDEF RX_D4}overload; {$ENDIF}
{$IFDEF RX_D4}
function FileUnlock(Handle: THandle; Offset, LockSize: Int64): Integer; overload;
{$ENDIF}
...
begin
  ...
  begin
    ...
    TBrowseFolderDlg(Data).FDefWndProc := Pointer(SetWindowLong(Wnd, GWL_WNDPROC,
      {$IFDEF RX_D16}LONG_PTR{$ELSE}Longint{$ENDIF}(TBrowseFolderDlg(Data).FObjectInstance)));
...
begin
  ...
  try
    ...
    begin
      ...
      lpfn := ExplorerHook;
      lParam := {$IFDEF WIN64}UIntPtr{$ELSE}LongInt{$ENDIF}(Self);
...
function FileLock(Handle: THandle; Offset, LockSize: Int64): Integer;
begin
...
function FileUnlock(Handle: THandle; Offset, LockSize: Int64): Integer;
begin
...
function LockFile(Handle: THandle; StartPos, Length: LongInt;
