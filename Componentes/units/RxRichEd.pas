...
{$DEFINE LAST_RICHED} {for backward compatibility}
{$R-}
...
begin
  ...
  begin
    ...
    begin
      ...
      P^ := $0D; Inc(P);
      P^ := $00; Inc(P);
      // Přidání LF (0A 00) ihned po CR
      P^ := $0A; Inc(P);
      P^ := $00; Inc(P);
      Inc(I, 2); // Přeskočení přes CR nebo LF
    end
    else
    begin
      // Kopírování ostatních bajtů
      P^ := Source[I]; Inc(P);
      Inc(I);
      if I < Start + Len then
      begin
  ...
  // Výpočet a vrácení počtu zapsaných bajtů
...
begin
  ...
  // Kopírování znaků z jednoho řetězce do druhého
  while PSource^ <> #0 do
  begin
    // Pokud je aktuální znak nový řádek, vložíme za ním CRLF
    if PSource^ = #13 then
    begin
      PDest^ := PSource^;
      Inc(PDest);
    ...
    begin
      // Kopírování aktuálního znaku do cílového řetězce
      PDest^ := PSource^;
      Inc(PDest);
      Inc(PSource);
    end;
  ...
  // Výpočet délky upraveného řetězce
