...
uses 
  SysUtils, Dialogs, ExtCtrls, DbConsts, {$IFDEF RX_D5}RxAppUtils{$ELSE}AppUtils{$ENDIF}, RxVCLUtils, // Alex
  RxDBUtils, {$IFNDEF RX_D3}BdeUtils, {$ENDIF}RxPickDate, RxCalc, RxMaxMin, RxResConst, // Polaris
  {$IFDEF RX_D7}Themes, {$ENDIF}
  {$IFDEF RX_D16}Styles, {$ENDIF}
...
begin
  ...
  begin
    with Selection do
      TempRect := BoxRect(Left, Top, Right, Bottom);
